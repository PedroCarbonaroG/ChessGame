...
public class ChessMatch {
    ...
    private void undoMove(Position source, Position target, Piece capturedPiece) {
        ...
        if (p instanceof Pawn) {
            if (source.getColumn() != target.getColumn() && capturedPiece == enPassantVulnerable) {
                ...
                Position pawnPosition;
                pawnPosition = (p.getColor() == Color.WHITE) ? new Position(3, target.getColumn()) : new Position(4, target.getColumn());
    ...
    private boolean testCheckMate(Color color) {
        ...
        List<Piece> playerInCheck = piecesOnTheBoard.stream().filter(x -> ((ChessPiece)x).getColor() == color).collect(Collectors.toList());
        for (Piece p : playerInCheck) {
            boolean[][] mat = p.possibleMoves();
            for (int i = 0; i < board.getRows(); i++) {
                for (int j = 0; j < board.getColumns(); j++) {
                    if (mat[i][j]) {
